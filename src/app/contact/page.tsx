    ...
    <main className="min-h-screen bg-parchment">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-stone-100 flex flex-col items-center text-center group hover:border-saffron transition-colors">
            ...
            <Link href="https://wa.me/919876543210" className="w-full">
              <Button variant="outline" className="w-full rounded-xl border-stone-200 hover:border-saffron hover:text-saffron">
                Open WhatsApp →
              </Button>
            </Link>
          </div>
                ...
                {!isSuccess ? (
                  ...
                  >
                    <div className="text-center space-y-2">
                      <h2 className="font-display text-3xl text-forest">Plan your trek</h2>
                      <p className="text-stone-500 text-sm">Tell us your mountain goals, and we'll help you reach them.</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        {/* Email */}
                        <div className="space-y-2">
                          <label className="text-xs uppercase tracking-widest font-bold text-stone-400">Email</label>
                          <input
                            {...register("email")}
                            className={cn(
                              "w-full bg-bone border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-saffron/50 transition-all",
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        {/* Phone */}
                        <div className="space-y-2">
                          <label className="text-xs uppercase tracking-widest font-bold text-stone-400">Phone</label>
                          <input
                            {...register("phone")}
                            className={cn(
                              "w-full bg-bone border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-saffron/50 transition-all",
                        ...
                        {/* Trek Interest */}
                        <div className="space-y-2">
                          <label className="text-xs uppercase tracking-widest font-bold text-stone-400">Which trek interests you?</label>
                          <select
                            {...register("trek")}
                            className={cn(
                              "w-full bg-bone border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-saffron/50 transition-all",
                          ...
                          >
                            ...
                            {trekData.map(t => <option key={t.slug} value={t.name}>{t.name}</option>)}
                            <option value="Not sure yet">Not sure yet</option>
                          </select>
                          {errors.trek && <p className="text-[11px] text-rose-500">{errors.trek.message}</p>}
                        </div>
                      </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        {/* Timing */}
                        <div className="space-y-2">
                          <label className="text-xs uppercase tracking-widest font-bold text-stone-400">When are you thinking?</label>
                          <select
                            {...register("timing")}
                            className={cn(
                              "w-full bg-bone border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-saffron/50 transition-all",
                          ...
                          {errors.timing && <p className="text-[11px] text-rose-500">{errors.timing.message}</p>}
                        </div>

                        {/* Group Size */}
                        <div className="space-y-2">
                          <label className="text-xs uppercase tracking-widest font-bold text-stone-400">Group size</label>
                          <select
                            {...register("groupSize")}
                            className={cn(
                              "w-full bg-bone border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-saffron/50 transition-all",
                              errors.groupSize && "border-rose-500 ring-rose-500/10"
                            )}
                          >
                          ...
                          {errors.groupSize && <p className="text-[11px] text-rose-500">{errors.groupSize.message}</p>}
                        </div>
                      </div>

                      {/* Message */}
                      <div className="space-y-2">
                        <label className="text-xs uppercase tracking-widest font-bold text-stone-400">Anything specific you want to know?</label>
                        <textarea
                          {...register("message")}
                          rows={4}
                          className={cn(
                  ...
                  >
                    ...
                    <h2 className="font-display italic text-4xl text-forest leading-tight">
                      Thank you, {userName}. <br /> We'll reach out within 24 hours.
                    </h2>
                    <p className="text-stone-500 text-lg italic">Your trail is waiting.</p>
                    <div className="pt-8">
      ...
      <section className="py-32 px-6 container mx-auto max-w-4xl">
        <div className="text-center mb-16 space-y-4">
          <h2 className="font-display text-4xl text-forest">Common Questions</h2>
          <p className="text-stone-500 max-w-xl mx-auto">Everything you need to know before you lace up your boots.</p>
        </div>
            ...
            >
              ...
              >
                <span className={cn(
                  ...
                  activeFaq === idx ? "text-saffron" : "text-forest group-hover:text-saffron"
                )}>
                  {faq.q}
                </span>
