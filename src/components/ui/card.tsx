...
// ─── Card ──────────────────────────────────────────────────────────────────
// Inherits bg-card and text-card-foreground from the design token system.
// On the parchment theme: bg-card = #F7F3ED (bone), text = #1A140F (monk-dark).
// Override with className as needed (e.g. bg-forest text-white for dark cards).
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Border — uses design token (warm, not cold gray)
        "ring-1 ring-border",
        // Padding variants
        "py-4 data-[size=sm]:gap-3 data-[size=sm]:py-3",
        // Image handling
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-xl border-t border-border",
        "bg-muted/40 p-4",
