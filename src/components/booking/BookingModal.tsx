...
import { CheckCircle2, Loader2, Mountain, Users, Calendar, Mail, Phone, User, Users2 } from "lucide-react";
...
export const BookingModal = ({ trek }: BookingModalProps) => {
  ...
  return (
    <Dialog open={bookingDrawerOpen} onOpenChange={(open) => !open && handleClose()}>
          ...
          <div className="bg-monk-dark p-8 text-white hidden md:flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="space-y-8 relative z-10">
              <div className="space-y-2">
                <p className="text-saffron text-[10px] uppercase tracking-[0.4em] font-bold">Booking for</p>
                <h3 className="font-display text-2xl italic leading-tight">{trek.name}</h3>
              </div>

              <div className="space-y-4 pt-8 border-t border-white/10">
                <div className="flex items-center gap-3 text-white/60">
                  <Mountain className="size-4 text-saffron" />
                  <span className="text-xs font-light tracking-wide">{trek.altitude}</span>
                </div>
                <div className="flex items-center gap-3 text-white/60">
                  <Users className="size-4 text-saffron" />
                  <span className="text-xs font-light tracking-wide">Small Group Journey</span>
                </div>
              </div>
            </div>
            ...
            <AnimatePresence mode="wait">
              {isSuccess ? (
                ...
                >
                  ...
                  <h3 className="font-display text-3xl text-forest italic">Reservation Initiated</h3>
                  <p className="text-stone-500 max-w-xs mx-auto font-light">
                    {state?.message || "Your request has been received. Our expedition monk will contact you within 4 hours."}
                  </p>
                  <Button 
                    onClick={handleClose}
                    variant="outline"
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase tracking-widest font-bold text-stone-400">Select Date</Label>
                      <Select onValueChange={(v) => setValue("date", v)}>
                        <SelectTrigger className="h-12 rounded-xl bg-white/50 border-stone-200">
                          <div className="flex items-center gap-2">
                            <Calendar className="size-4 text-stone-300" />
                            <SelectValue placeholder="Select a date" />
                          </div>
                        </SelectTrigger>
                        <SelectContent className="bg-parchment rounded-xl border-stone-200">
                          {trek.availableDates.map((d) => (
                            <SelectItem key={d.date} value={d.date} className="focus:bg-saffron/10 focus:text-forest">
                              {d.date} ({d.spots} spots)
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      {errors.date && <p className="text-[10px] text-rose-500 font-bold">{errors.date.message}</p>}
                    </div>
                    ...
                    <div className="sm:col-span-2 space-y-2">
                      ...
                      {errors.guests && <p className="text-[10px] text-rose-500 font-bold">{errors.guests.message}</p>}
                    </div>
                  </div>
                  ...
                  {state?.success === false && (
                    <p className="text-xs text-rose-500 text-center font-bold">{state.message}</p>
                  )}
                </motion.form>
              )}
            </AnimatePresence>
